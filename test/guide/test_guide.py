...
def stringify_list(lst):
    '''
        converts list items to coma separated string
    '''
    return ",".join(lst).replace(" ", "-").lower()

def load_files(path, extension=".md"):
    '''
        loads all files in 'path' ending with 'extension'. Stores the 
        filename (without extension), its content as a string and the 
        other files that are given as list into a dictionary. For each
        file, this dictionary is made. Finally returned as a list of
        all dictionary. 
    '''
    data = []
